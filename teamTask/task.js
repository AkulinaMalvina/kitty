...
function getBugsANDCrutchs(team) {
  ...
  for (let employee in team) {
    ...
    if (employee === "trainee") {
      bugs = bugs + team[employee] * 1 * 3; // здесь всё верно
      сrutchs = сrutchs + team[employee] * 0 * 3; // здесь всё верно

    };

    if (employee === "juniorBackend") {
      bugs = bugs + team[employee] * 3 * 3; // здесь всё верно
      сrutchs = сrutchs + team[employee] * 1 * 3; // здесь всё верно
      if (team[employee] >= 2)
        return 'Астрологи предсказали квартал багов, количество багов увеличилось втрое!'
    };

    if (employee === "middleBackend") {
      bugs = bugs + team[employee] * 2 * 3; // здесь всё верно
      сrutchs = сrutchs + team[employee] * 3 * 3; // здесь всё верно
    };
    ...
    if (employee === "juniorFrontend") {
      bugs = bugs + team[employee] * 3 * 3; // здесь всё верно
      сrutchs = сrutchs + team[employee] * 4 * 3; // здесь всё верно
    };
    ...
    if (employee === "teamLead") {
      bugs = bugs + team[employee] * 1; // здесь всё верно
      сrutchs = сrutchs + team[employee] * 0 * 3; // здесь всё верно
    };
  }
